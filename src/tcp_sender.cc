...
void TCPSender::push( const TransmitFunction& transmit )
{
  ...
  else if ( window_size_ == 0 && last_out_index_next_ - first_out_index_ == 0 )
    left_space = 1;
  else
    left_space = 0;
  ...
  while ( left_space >= TCPConfig::MAX_PAYLOAD_SIZE && s.size() > TCPConfig::MAX_PAYLOAD_SIZE ) {
...
{
  return { Wrap32::wrap( last_out_index_next_, isn_ ), false, "", false, input_.has_error() };
}
...
void TCPSender::receive( const TCPReceiverMessage& msg )
{
  ...
  while ( outstanding_segments_.size()
          && first_out_index_ + outstanding_segments_.front().sequence_length()
               <= left_.unwrap( isn_, first_out_index_ ) ) {
    first_out_index_ += outstanding_segments_.front().sequence_length();
